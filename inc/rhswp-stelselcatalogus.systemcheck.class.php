...
class StelselplaatSystemCheck {
    ...
    public function __construct() {
        $this->options = get_site_option( 'stelselplaat_systemcheck' );
    }
    ...
    private function dismissMessages() {
        if ( isset( $_REQUEST['dismissMessage'] ) && isset( $_REQUEST['_wpnonce'] ) ) {
            ...
            if ( wp_verify_nonce( $nonce, "stelselplaat-dismiss-{$key}" ) ) {
                $this->options[$key] = false;
                update_site_option( 'stelselplaat_systemcheck', $this->options );
            }
        }
    }
    ...
    private function checkWordPressVersion() {
        ...
        if ( !function_exists( 'wp_enqueue_media' ) ) {
            $error = "Stelselplaat requires WordPress 3.5 or above. Please upgrade your WordPress installation.";
    ...
    private function printMessage( $message, $key ) {
        $nonce = wp_create_nonce( "stelselplaat-dismiss-{$key}" );
